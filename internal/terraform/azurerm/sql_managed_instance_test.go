...
func TestSQLManagedInstance(t *testing.T) {
	t.Skip("`azurerm_sql_managed_instance` resource is removed from the latest provider")
	t.Parallel()
	if testing.Short() {
	...
	testcase.GoldenFileResourceTestsWithOpts(t, "sql_managed_instance_test", opts)
}
