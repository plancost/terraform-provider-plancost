...
func TestDataFactoryIntegrationRuntimeManaged(t *testing.T) {
	t.Skip("skipping test because the resource has been removed from the provider")
	t.Parallel()
	if testing.Short() {
	...
	testcase.GoldenFileResourceTestsWithOpts(t, "data_factory_integration_runtime_managed_test", opts)
}
