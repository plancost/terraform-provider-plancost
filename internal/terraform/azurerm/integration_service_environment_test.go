...
func TestAzureRMAIntegrationServiceEnvironment(t *testing.T) {
	t.Skip("`azurerm_integration_service_environment` resource is removed from the latest provider")
	t.Parallel()
	if testing.Short() {
	...
	testcase.GoldenFileResourceTestsWithOpts(t, "integration_service_environment_test", opts)
}
