...
import (
	...
	"github.com/plancost/terraform-provider-plancost/internal/terraform"
	...
	"github.com/plancost/terraform-provider-plancost/internal/hclparser/hcl/modules"
	"github.com/plancost/terraform-provider-plancost/internal/hclparser/providers/terraform/aws"
	"github.com/plancost/terraform-provider-plancost/internal/hclparser/providers/terraform/azure"
	"github.com/plancost/terraform-provider-plancost/internal/hclparser/providers/terraform/google"
...
func NewParser(moduleLoader *modules.ModuleLoader, includePastResources bool) *Parser {
	return &Parser{
...
func (p *Parser) createParsedResource(d *schema.ResourceData, u *schema.UsageData) parsedResource {
	if registryItem, ok := (*terraform.ResourceRegistryMap)[d.Type]; ok {
...
func (p *Parser) parseResourceData(isState bool, confLoader *ConfLoader, providerConf, planVals, vars gjson.Result) map[string]*schema.ResourceData { //nolint:unparam
...
func getSpecialContext(d *schema.ResourceData) map[string]interface{} { //nolint:unused
...
func resourceRegion(d *schema.ResourceData) string {
	...
	regionFunc := terraform.ResourceRegistryMap.GetRegion(d.Type)
	if regionFunc != nil {
...
func (p *Parser) parseReferences(resData map[string]*schema.ResourceData, confLoader *ConfLoader) {
	...
	for _, d := range resData {
		...
		if f := terraform.ResourceRegistryMap.GetDefaultRefIDFunc(d.Type); f != nil {
		...
		if f := terraform.ResourceRegistryMap.GetCustomRefIDFunc(d.Type); f != nil {
	...
	for _, d := range resData {
		refAttrs := terraform.ResourceRegistryMap.GetReferenceAttributes(d.Type)

		for _, attr := range refAttrs {
			found := p.parseConfReferences(resData, confLoader, d, attr, terraform.ResourceRegistryMap)
			...
			for _, refVal := range d.Get(attr).Array() {
				...
				if ok {
					...
					for _, ref := range idRefs {
						reverseRefAttrs := terraform.ResourceRegistryMap.GetReferenceAttributes(ref.Type)
...
func (p *Parser) parseConfReferences(resData map[string]*schema.ResourceData, confLoader *ConfLoader, d *schema.ResourceData, attr string, registryMap *terraform.RegistryItemMap) bool {
