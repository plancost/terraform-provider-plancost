...
type flagStringSlice []string //nolint:unused

func (v *flagStringSlice) String() string { return "" } //nolint:unused
func (v *flagStringSlice) Set(raw string) error { //nolint:unused
	*v = append(*v, raw)
	return nil
}

type vars struct { //nolint:unused
	files []string
	vars  []string
}

var spaceReg = regexp.MustCompile(`\s+`) //nolint:unused

func varsFromPlanFlags(planFlags string) (vars, error) { //nolint:unused
...
func NewHCLProvider(path string, config *HCLProviderConfig, opts ...hcl.Option) (*HCLProvider, error) {
	if config == nil {
		config = &HCLProviderConfig{}
	}
	...
	envMatcher := hcl.CreateEnvFileMatcher(nil, nil)
	moduleLoader := modules.NewModuleLoader(modules.ModuleLoaderOptions{
		CachePath: path,
		HCLParser: modules.NewSharedHCLParser(),
		Logger:    logging.Logger,
	})

	rootPath := hcl.RootPath{
		StartingPath: path,
		DetectedPath: path,
	}

	p := hcl.NewParser(rootPath, envMatcher, moduleLoader, logging.Logger, options...)

	return &HCLProvider{
		Parser:         p,
		planJSONParser: NewParser(moduleLoader, false),
		logger:         logging.Logger,
...
func tryAbs(initialPath string) string { //nolint:unused
...
func (p *HCLProvider) LoadResources(usage schema.UsageMap) ([]*schema.Project, error) {
	...
	j := p.LoadPlanJSON()
	if j.Error != nil {
		return []*schema.Project{p.newProject(j)}, nil //nolint:nilerr
	}
...
func (p *HCLProvider) LoadPlanJSON() HCLProject {
	...
	if module.Error == nil {
		...
		if os.Getenv("PLANCOST_JSON_DUMP") == "true" {
			targetPath := fmt.Sprintf("%s-out.json", strings.ReplaceAll(module.Module.ModulePath, "/", "-"))
			targetDir, ok := os.LookupEnv("PLANCOST_JSON_DUMP_PATH")
			if ok {
...
func (p *HCLProvider) getResourceOutput(block *hcl.Block, moduleSourceURL string) ResourceOutput { //nolint:unparam
	...
	for _, b := range block.Children() {
		...
		if v, ok := jsonValues[key]; ok {
			...
			if s, ok := v.([]interface{}); ok {
				jsonValues[key] = append(s, childValues)
			}
			continue
		}
