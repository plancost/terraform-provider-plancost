...
func Unmarshal(input types.Dynamic, out interface{}) error {
...
func ToDynamic(input interface{}) (types.Dynamic, error) {
	data, err := json.Marshal(input)
	if err != nil {
		return types.Dynamic{}, fmt.Errorf("marshaling failed: value: %+v, err: %+v", input, err)
	}
	dynamicVal, err := FromJSONImplied(data)
	if err != nil {
		return types.Dynamic{}, fmt.Errorf("invalid dynamic value: value: %+v, err: %+v", input, err)
	}
