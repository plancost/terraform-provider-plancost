...
func (d *ResourceData) Get(key string) gjson.Result {
	// Note-plancost: mapping region -> location for azure resources
	if key == "region" && d.RawValues.Get("location").String() != "" {
		locationV := d.RawValues.Get("location").String()
		locationV = toAzureCLIName(locationV)
		return gjson.Parse(`"` + strings.Clone(locationV) + `"`)
	}
	if key == "region" && d.Region != "" {
		locationV := toAzureCLIName(d.Region)
		return gjson.Parse(`"` + strings.Clone(locationV) + `"`)
	}
	// Note-plancost:
	if d.RawValues.Get(key).String() == "" || d.RawValues.Get(key).Type == gjson.Null {
		return gjson.Result{}
	}
